  ...
  const addToCart = async (product) => {
    try {
      const response = await fetch(
        "https://script.google.com/macros/s/AKfycbz4IvWiLtjvURYi44y50ps1cMQMw3smg06V6qa1KSjzpIFplsMWd_WYM5n_zc13r74QeQ/exec",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        console.log("Product added to cart successfully with id:", result.id);
      } else {
    ...
    const handleAddToCart = () => {
      ...
      console.log("Image URL:", imgUrl); // Log the image URL to verify it
      addToCart({
        name: product.name,
        status: product.status,
        price: product.price,
        quantity: 1,
        img: imgUrl, // Ensure img is a URL string
      });
    };
    ...
    return (
      <SafeAreaView className="flex-1 ">
        <View className="flex-1">
          ...
          >
            <View className="w-full h-[200px] rounded-lg shadow-sm p-1">
              <Image
            ...
            <View className="w-full py-3 px-1 flex-row justify-between border-b-[1px] border-gray-300">
              <View>
                <Text className="text-base font-pmedium mr-2">Share with:</Text>
            ...
            <View>
              <View className="flex flex-row justify-between w-full items-center mb-4">
                ...
                <Link href="/details/Reviews" asChild>
